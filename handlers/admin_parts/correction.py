...
CORRECTION_CONFIGS = {
    ...
    "fuel_consumption": {
        ...
        "help_text": "\n\n💡 <i>Значення за замовчуванням: {:.2f} л/год (з .env).\nКорекція перевизначає це значення для розрахунку споживання палива.</i>".format(
    ...
    async def set_handler(cb: types.CallbackQuery, state: FSMContext):
        ...
        if st.get("status") == "ON":
            return await cb.answer(
                "⛔ Корекції заборонені під час активної зміни. Спочатку натисніть СТОП.", show_alert=True
            )
