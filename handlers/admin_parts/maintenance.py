...
- Maintenance history
- Motor hours correction
- Per-generator tracking (main/emergency)
"""
...
class SetHoursForm(StatesGroup):
    """FSM states for motor hours correction."""
...
class MaintenanceForm(StatesGroup):
    """FSM states for maintenance execution."""
    generator = State()  # Який генератор
    action = State()  # Тип ТО
...
def format_mnt_status(generator_id: str) -> str:
    ...
    Args:
        generator_id: Generator identifier (main/emergency)

    Returns:
        Formatted status text with warnings
    """
    ...
    def fmt_line(name: str, icon: str, current: float, needed: float, interval: int) -> str:
        """Format single maintenance line.

        Args:
            name: Maintenance type name
            icon: Icon emoji
            current: Current hours
            needed: Hours until maintenance
            interval: Maintenance interval

        Returns:
            Formatted line with status
        """
        if needed <= 0:
            status = "🔴 ТЕРМІНОВЕ ТО!"
        elif needed <= 10:
...
async def mnt_view(cb: types.CallbackQuery, state: FSMContext) -> None:
...
async def mnt_choose_generator(cb: types.CallbackQuery, state: FSMContext) -> None:
...
async def mnt_choose_action(cb: types.CallbackQuery, state: FSMContext) -> None:
    """Вибір типу ТО.
...
async def mnt_confirm_action(cb: types.CallbackQuery, state: FSMContext) -> None:
...
async def mnt_history_menu(cb: types.CallbackQuery) -> None:
...
async def mnt_show_history(cb: types.CallbackQuery) -> None:
...
async def mnt_hours_choose_gen(cb: types.CallbackQuery, state: FSMContext) -> None:
...
async def ask_hours(cb: types.CallbackQuery, state: FSMContext) -> None:
...
async def save_hours(msg: types.Message, state: FSMContext) -> None:
    """Збереження нового значення мотогодин.

    Args:
        msg: Message with new hours value
        state: FSM context
    """
