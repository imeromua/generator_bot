...
def format_mnt_status(generator_id: str) -> str:
    ...
    mnt_line = fmt_line(
        "Планове ТО",
        "🔧",
        stats['total_hours'] % config.MAINTENANCE_INTERVAL,
...
async def mnt_choose_generator(cb: types.CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text("🛠 <b>Виконання ТО</b>\n\nОберіть генератор:", reply_markup=markup)
    ...
    kb = [
        [
            types.InlineKeyboardButton(
                text=f"🛢 Заміна мастила (↻ {stats['last_oil']:.1f} год)", callback_data="mnt_action_oil"
            )
        ],
        [
            types.InlineKeyboardButton(
                text=f"🕯 Заміна свічок (↻ {stats['last_spark']:.1f} год)", callback_data="mnt_action_spark"
            )
        ],
...
async def mnt_history_menu(cb: types.CallbackQuery):
    ...
    await cb.message.edit_text("📜 <b>Історія ТО</b>\n\nОберіть генератор:", reply_markup=markup)
...
async def mnt_hours_choose_gen(cb: types.CallbackQuery, state: FSMContext):
    ...
    kb = [
        [
            types.InlineKeyboardButton(
                text=f"🔋 Основний ({main_stats['total_hours']:.1f} год)", callback_data="mnt_hours_main"
            )
        ],
        [
            types.InlineKeyboardButton(
                text=f"⚠️ Аварійний ({emerg_stats['total_hours']:.1f} год)", callback_data="mnt_hours_emergency"
            )
        ],
        [types.InlineKeyboardButton(text="🔙 Назад", callback_data="mnt_menu")],
    ]
    ...
    await cb.message.edit_text("⏱ <b>Корекція мотогодин</b>\n\nОберіть генератор:", reply_markup=markup)
...
async def save_hours(msg: types.Message, state: FSMContext):
    ...
    try:
        ...
        try:
            await msg.bot.edit_message_text(
                chat_id=msg.chat.id, message_id=bot_message_id, text=txt, reply_markup=maintenance_menu_new()
            )
        except TelegramBadRequest:
