...
TOKEN = os.getenv("ADMIN_BOT_TOKEN", "8275096361:AAH_aaueD-T1PLSkRkK_qwYizKB6ATkS91k")
ADMIN_ID = int(os.getenv("ADMIN_BOT_ADMIN_ID", "1962821395"))
...
async def pip_menu(message: types.Message):
    ...
    await message.answer("📦 <b>Менеджер пакетів Python</b>\nОберіть дію:", reply_markup=kb, parse_mode="HTML")
...
async def pip_install(cb: CallbackQuery):
    msg = await cb.message.answer("⏳ <i>Запускаю pip install... Це може зайняти час.</i>", parse_mode="HTML")
...
async def pip_outdated(cb: CallbackQuery):
    msg = await cb.message.answer("⏳ <i>Перевіряю оновлення...</i>", parse_mode="HTML")
    python_exe = sys.executable
...
async def get_status(message: types.Message):
    ...
    await message.answer(
        f"{icon} <b>System Status</b>\n<blockquote expandable>{safe_html(status_raw[:3000])}</blockquote>",
        parse_mode="HTML"
    )
...
async def edit_env_var(cb: CallbackQuery, state: FSMContext):
    ...
    kb = []
    if quick_replies:
        kb.append([InlineKeyboardButton(text=v, callback_data=f"set_val:{v}") for v in quick_replies])
...
async def save_restart(cb: CallbackQuery):
    ...
    if status == "active":
        await cb.message.answer("✅ <b>Сервіс успішно перезапущено!</b>", parse_mode="HTML")
    else:
